...
- After dropping `t(key, 'literal')` fallbacks, tests that mock `t: (key, fallback) => fallback ?? key` keep asserting English and go red. Switch those mocks to `t: (key) => key` and assert the catalogue key.
...
- 2026-08-31: #1154 slices 13–15 on `cursor/1154/calls-forms-i18n` (from remaining-chrome / #1242). Call widgets, form/modal defaults, composer toolbar, and rescan extras now use keys-only `t()`. Gate: `callsFormsI18n` 28 PASS, `i18n.test.ts` 31 PASS, `test:unit` 3735 PASS, `lint:scripts` PASS, local only. Uncommitted; stacked PR + #1154 comment pending user ask. Still out of repo: API `agency.name`/`topic.name`, ConsultingTypeService `option.label`, Ukrainian UI locale. Do not merge the stack or close #1154 without asking.
