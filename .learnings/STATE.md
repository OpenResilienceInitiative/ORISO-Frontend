...
- 2026-09-01: #1213 item 1 — profile agency card opens Impressum/Datenschutz in `LegalLinkModal` `scope="agency"` via `DepartmentLegalSection` `variant="modal"`. Footer platform-only already in PR #1220, pinned with `LegalLinks.test.tsx`. Gate: targeted 20 PASS, `lint:scripts` PASS, `test:unit` 3635 PASS, `build` compiled (postbuild FAIL on local `.env` realm). Next: Pre-Dev reviewer check of agency card modal vs footer URLs.
