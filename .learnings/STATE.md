...
- On a group row only `showChatSettings` can be true: `isOwnEditableSession` requires `isSession` (false for group chats) and `isTeamDiscussionAvailable` returns false when `isGroup`.
...
- 2026-08-27: #1189 follow-up — the first Job 2 commit was a no-op: the menu markup sat in the main return, which group rows never reach. Fixed by extracting `SessionListItemMenu` and rendering it from both branches, and by adding the render-level tests that were missing (`SessionListItemComponent.test.tsx`, `SessionListItemMenu.test.tsx`). Red proven by emptying the group cell again: 2 of 4 reachability tests fail, the two absence-assertions stay green. Gate: `test:unit` 3462 PASS, `lint:scripts` PASS, `build` PASS, `lint:style` FAIL on pre-existing SCSS only (0 SCSS files touched). PR #1205 still draft pending a decision on marking it ready.
