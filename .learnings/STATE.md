...
- A #1154 leftovers file-list is not a full-platform scan. After each slice, walk all production `t`/`tr`/`translate` second-arg string fallbacks — session header/menu/list and `translateWithFallback` wrappers were still snapping after #1241.
...
- 2026-08-31: #1154 leftovers (#1241) was not the last in-repo slice. A full-platform rescan found 117 `t`/`tr`/`translate` string fallbacks still in session header/menu/list, waiting countdown, notifications, legal, handover carousel, and opening-hours weekdays. Fixed on `cursor/1154/remaining-chrome-i18n` (stacked on leftovers). Gate so far: remainingChrome 23 PASS, countdown 10 PASS, i18n guard 31 PASS, local only. Still out of repo: API `agency.name`/`topic.name`, ConsultingTypeService `option.label`, Ukrainian UI. Still in repo but not `t()` fallbacks: call widgets and form-primitive aria-labels (raw English/German).
