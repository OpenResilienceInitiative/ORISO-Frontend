...
test('capture figma and app composer', async ({ browser }) => {
	...
	await figma.goto(
		'https://www.figma.com/design/Y7PJA0yRXm8R8i49kFnnvP/ORISO?node-id=84-14745&t=gfdDHfZuUusXoUZQ-0',
		{ waitUntil: 'domcontentloaded' }
	);
	await figma.waitForTimeout(8000);
	await figma.screenshot({
		path: '/tmp/figma-node-full.png',
		fullPage: true
	});

	const app = await context.newPage();
	await app.goto('https://app.oriso.org', { waitUntil: 'domcontentloaded' });
	...
	await app.goto(
		'https://app.oriso.org/sessions/consultant/sessionView/session/102318',
		{ waitUntil: 'domcontentloaded' }
	);
