// FORK of lib/story-imports.mjs — declared in cfg.libOverrides.
//
// Why: the preview compile needs the same two loader behaviours the app's own
// webpack/vite config has, and neither is expressible through
// cfg.storyImports.* (which only forces shim/bundle routing and a loader map):
//
//   1. Extensionless SCSS side-effect imports (`import './chatMenuDropdown.styles'`)
//      — the CRA convention. esbuild has no resolveExtensions knob on this seam,
//      and the seam returns {plugins, loaders} only, so it is added as a plugin.
//   2. The CRA dual-export SVG (`import { ReactComponent as Icon } from './x.svg'`).
//      STORY_LOADERS maps .svg to dataurl, which has no named export, so any
//      story importing an icon that way failed to compile.
//
// Everything else is byte-identical to the staged original; the two plugins are
// appended to the returned array at the bottom of storyImportPlugins().
...
// ── ORISO fork additions ────────────────────────────────────────────────
  // Extensionless style imports: only claim a specifier when a stylesheet
  // sibling exists AND no JS/TS sibling does, so normal module resolution is
  // untouched. The resolved .scss then hits STORY_LOADERS' 'empty' loader —
  // correct, because component styles ship via _ds_bundle.css.
  const extensionlessStyles = {
    name: 'oriso-extensionless-styles',
    setup(b) {
      b.onResolve({ filter: /^\.{1,2}\// }, (a) => {
        if (a.namespace !== 'file' && a.namespace !== '') return null;
        const base = resolve(a.resolveDir, a.path);
        for (const e of ['.ts', '.tsx', '.js', '.jsx', '.mjs', '.cjs', '.json']) {
          if (existsSync(base + e)) return null;
        }
  ...
  // CRA dual-export SVG for story-side imports. svgr is resolved from the
  // REPO's node_modules by absolute path (the fork's own node_modules symlink
  // points at the converter deps, which don't carry svgr). If it is missing the
  // plugin stays out of the way rather than shipping a wrong render.
  const svgDualExport = {
    name: 'oriso-story-svg-dual',
    setup(b) {
      let svgr = null;
      let tried = false;
      b.onResolve({ filter: /\.svg$/ }, async (a) => {
        if (a.kind === 'url-token' || a.pluginData === 'oriso-svg') return null;
        const r = await b.resolve(a.path, {
          resolveDir: a.resolveDir,
          kind: a.kind,
          pluginData: 'oriso-svg',
        });
        if (r.errors.length) return r;
        return { path: r.path, namespace: 'oriso-svg' };
      });
      b.onLoad({ filter: /.*/, namespace: 'oriso-svg' }, async (a) => {
        if (!tried) {
          tried = true;
          try {
            svgr = (await import(`${pkgDir}/node_modules/@svgr/core/dist/index.js`)).transform;
          } catch { svgr = null; }
        }
        const raw = readFileSync(a.path, 'utf8');
        const url = `data:image/svg+xml;base64,${Buffer.from(raw).toString('base64')}`;
        if (!svgr) {
          return { contents: `export default ${JSON.stringify(url)};export const ReactComponent=()=>null;`, loader: 'js' };
        }
        const component = await svgr(
          raw,
          { plugins: ['@svgr/plugin-jsx'], exportType: 'named', namedExport: 'ReactComponent', jsxRuntime: 'classic' },
          { componentName: 'ReactComponent', filePath: a.path },
        );
        return {
  ...
  return {
    plugins: [dsShim, storybookStubPlugin(), consoleStub, policyRedirect, extensionlessStyles, svgDualExport],
