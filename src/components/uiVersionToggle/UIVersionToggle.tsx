...
export const UIVersionToggle = () => {
	...
	const toggleUI = () => {
		...
		if (newVersion === 'new') {
			...
			const homeserverUrl =
				process.env.REACT_APP_MATRIX_HOMESERVER_URL ||
				`http://${currentHost}:8008`;
			...
			if (matrixUserId && matrixAccessToken) {
				...
				console.log(
					'✅ Matrix credentials stored in cookies for Element SSO'
				);
				...
				console.log(
					'📍 Cookie domain:',
					cookieDomain || 'current host only'
				);
			} else {
				console.warn(
					'⚠️ No Matrix credentials found - Element will require manual login'
				);
			}
			...
			const elementUrl =
				process.env.REACT_APP_ELEMENT_URL ||
				`http://${currentHost}:8087`;
