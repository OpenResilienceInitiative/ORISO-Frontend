...
describe('session list visual contracts', () => {
	it('keeps every populated session card independently rounded', () => {
		...
		expect(css).toMatch(
			/\.sessionsListItem\s*\{[^}]*margin-bottom:\s*6px;/s
		);
	});
	...
	it('reserves side hairlines for the empty state and exposes the dragbar', () => {
		...
		expect(css).toMatch(
			/\.sessionsList__emptyState[^{}]*\{[^}]*border-left:\s*1px solid #fff;[^}]*border-right:\s*1px solid #fff;/s
		);
		expect(css).toMatch(
			/\.sessionsList__resizeHandle[^{}]*\{[^}]*right:\s*-24px;[^}]*width:\s*24px;/s
		);
		expect(css).toMatch(
