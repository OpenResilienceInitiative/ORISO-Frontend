...
**Product decision (12.09.2026).** A supervision call is staff-to-staff and
does not inherit the client-facing `consultingType.isVideoCallAllowed` gate.
`resolveSupervisionCallFeatureGates` retains the global call master, the
audio/video masters and the dedicated
`feature{Audio,Video}CallsSupervisionChatsEnabled` flags.
