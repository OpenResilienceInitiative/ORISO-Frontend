...
Stage-only (not global): the supervision on/off switch in the channel card
is presentational — `supervisionActive` / `onToggleSupervision` are props of
`ChannelMenu`, `PanelHeader` and `ChannelSwitcherFab`; B2 wires them.
