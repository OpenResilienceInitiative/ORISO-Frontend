...
| Old desktop scroll FAB goes (D6)                            | `SessionItemComponent.tsx` `session__scrollToBottom` — removed in B2 (both sites + the `session.styles.scss` block)        | the composer toolbar's `composer-scroll-to-newest` is the one arrow; story (a) asserts no `.session__scrollToBottom` renders.                                                                                                                                                                                                                                                                                                |
...
| Second scroll-FAB site removed in B2 (review v10 / D6)                         | `messageSubmitInterfaceComponent.tsx` (`getElementsByClassName('session__scrollToBottom')`, ~1170) is gone together with the `SessionItemComponent.tsx` site (B2)                                                                                                              | Story (a) asserts no `.session__scrollToBottom` renders in EITHER pane; B2 removed both sites together.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
### Added on 09.09.2026 — Frank's finding \*1 (divider range) and the side room's calls

| Decision                                                              | Where it applies                                                                                                                                                                    | Value                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| --------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| The divider reaches 320 px on either side (\*1)                       | `stageLayout.ts` `STAGE_LAYOUT.MIN_PANE_DRAG_WIDTH: 320`, used by `clampPanelWidth` and by both `ResizableHandle` call sites (`SessionItemComponent`, `ConsultantSessionStage`)     | Frank measured "mindestens 487 px" — that is `.panelHeader__row` on the old floor: 520 px slot − 1 px `.sidePanel--inside` hairline − 2 × 16 px `$room-header-inset`. Two floors now: `MIN_PANE_WIDTH` (520) stays the AUTO floor, so D10's rail snap is untouched (lowering the single floor would have switched the snap off at 1280: 824 px card ÷ 2 = 412 ≥ 320); `MIN_PANE_DRAG_WIDTH` (320) is what the handle may reach. Stories (i)/(j) hold both extremes, (k) is the old 520 px minimum and measures the 487.    |
| A room header goes compact by PANE width, not by viewport width (\*1) | `sessionHeader/roomHeaderDensity.ts` (`HEADER_COMPACT_WIDTH: 480`), read by `SessionHeaderComponent` through a `react-use-measure` on `.sessionInfo` (`data-density`)               | At 1280 px the viewport says "desktop" while the pane behind the divider can be 320 px. Measured on the running stage: with the calls inline the main title column is `pane − 302` px, so at 320 the name had 20 px and was clipped by 161. Below 480 the header takes D8's phone form — calls into the kebab, avatar stack folded into "+N" — which is where D8's own "title keeps ≥ 40 % of the row" breaks (482) and the name starts being cut (480). `props.callsInMenu` still wins on top, so the phone is unchanged. |
| Supervision side room can start audio and video calls                 | `PanelHeader.actions` ← `chatStage/PanelCallActions.tsx`, rules in `chatStage/panelCallActionsState.ts`; wired in `SessionItemComponent` (app) and `ConsultantSessionStage` (stage) | The call goes against the SIDE room: `startRoomCall({ roomId: supervisionRoomId, … })`. `CallManager.startCall` creates the Element Call room from that id with a `restricted` join rule, so only the side room's members can join, and sends the invite back into it. `isGroup` is passed explicitly (`participants > 2`) because the CallManager's auto-detection filters power-level-10 members — the supervisors — out of its own head count.                                                                          |
| One call trigger, one call gate, one pair of glyphs                   | `call/startRoomCall.ts`, `call/callFeatureGates.ts`, `call/CallHeaderIcons.tsx` — all three lifted out of `SessionMenu.tsx`, which now calls them                                   | No second implementation: `SessionMenu` and the side room run the same steps (getUserMedia inside the click for mobile Safari, release, `callManager.startCall`) and ask the same tenant question. The side room asks it with `chatType: 'supervision'` — by WHICH room is on screen, where `SessionMenu` asks by WHO is looking (`props.isSupervisor`).                                                                                                                                                                   |
| Side room calls: row, kebab, disabled                                 | `panelCallActionsState.ts`                                                                                                                                                          | Row while the panel is ≥ `HEADER_COMPACT_WIDTH`, kebab below it and on the phone (same threshold as the main header). Alone in the room the buttons are DISABLED, never hidden, and say why in their tooltip. Both tenant flags off means the tenant switched calls off for the room — then nothing renders, as in `SessionMenu`. Stories (l)–(p).                                                                                                                                                                         |
| Story isolation for the persisted panel width                         | `ChatStage.stories.tsx` `meta.beforeEach` removes `chatStage_panelWidth`                                                                                                            | Storybook runs a file's stories in one iframe; a story that drags the divider used to decide the next story's layout. Found while writing (i)–(p) — (o) and (p) inherited 320 px from (m).                                                                                                                                                                                                                                                                                                                                 |

**Open for Frank (09.09.2026).** The side room's call buttons are gated with `consultingType.isVideoCallAllowed`, exactly like the main chat's (`SessionMenu.hasVideoCallFeatures`). That flag is about calls with CLIENTS; a supervision call is staff-to-staff and has its own tenant flags (`feature{Audio,Video}CallsSupervisionChatsEnabled`). The conservative reading was taken — an agency that does no calls at all gets none in the side room either. Say the word and the client-facing flag drops out of the side room's gate.
