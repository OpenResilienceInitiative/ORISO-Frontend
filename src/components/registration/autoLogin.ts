...
export const autoLogin = async ({
	...
	try {
		const { getMatrixAccessToken } =
			await import('../sessionCookie/getMatrixAccessToken');
...
export const redirectToApp = (gcid?: string, chatId?: number | null) => {
	const params = new URLSearchParams();
	if (gcid) {
		params.set('gcid', gcid);
	}
	if (chatId != null) {
	...
	window.location.href =
		appConfig.urls.redirectToApp + (query ? `?${query}` : '');
};
