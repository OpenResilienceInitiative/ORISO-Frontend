...
export const GroupCallWidget: React.FC = () => {
	...
	const setupElementCall = useCallback(() => {
		if (!callData) return;
		...
		try {
			...
			const homeserverUrl =
				client.getHomeserverUrl() || 'https://matrix.oriso.site';
			...
			console.log(
				'🔑 Passing Matrix credentials for auto-authentication'
			);
			...
			const sendCredentials = () => {
				if (iframeRef.current?.contentWindow) {
					console.log(
						'📤 Sending Matrix credentials to Element Call via postMessage'
					);
	...
	useEffect(() => {
		...
		console.log(
			'✅ Incoming group call moving to state',
			callState,
			'- setting up Element Call for receiver...'
		);
		setupElementCall();
	}, [callState, callData, elementCallUrl, setupElementCall]);
	...
	useEffect(() => {
		...
		setupElementCall();
	}, [callData, elementCallUrl, setupElementCall]);
	...
	const handleAnswer = () => {
		...
		if (!elementCallUrl) {
			console.log(
				'📞 Answer clicked, setting up Element Call immediately for receiver...'
			);
			setupElementCall();
		}
	};
	...
	const handleMouseDown = (e: React.MouseEvent) => {
		if (
			(e.target as HTMLElement).closest(
				'.btn-end-call, .btn-answer, .btn-decline, iframe'
			)
		)
			return;
		setIsDragging(true);
