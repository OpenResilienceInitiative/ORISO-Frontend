...
export const FloatingCallWidget: React.FC = () => {
	...
	useEffect(() => {
		...
		if (callData.isGroup) {
			console.log(
				'🚫 FloatingCallWidget: Skipping group call (handled by GroupCallWidget)'
			);
			return;
		}
		...
		const isStreamFresh =
			preRequestedTime && Date.now() - preRequestedTime < 5000; // Within 5 seconds
		...
		if (preRequestedStream && isStreamFresh) {
			...
			console.log(
				'⚠️ No pre-requested stream, requesting now (may fail on mobile)'
			);
			navigator.mediaDevices
				.getUserMedia({ video: callData.isVideo, audio: true })
				.then((stream) => {
