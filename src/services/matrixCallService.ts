...
class MatrixCallService {
	...
	public initialize(
		...
		this.client.on('Call.incoming' as any, (call: MatrixCall) => {
			console.log(
				'📞 Incoming Matrix WebRTC call from:',
				call.getOpponentMember()?.name
			);
	...
	public async startCall(options: MatrixCallOptions): Promise<MatrixCall> {
		...
		try {
			console.log(
				`📞 Starting ${options.isVideoCall ? 'video' : 'voice'} call in room:`,
				options.roomId
			);
			...
			if (!room) {
				...
				if (!room) {
					...
					console.error(
						'❌ Available rooms:',
						this.client.getRooms().map((r: any) => r.roomId)
					);
					throw new Error(
