...
const meta = {
	...
	args: { id: 'email-geaendert' as const, locale: 'de-sie' as const },
	tags: ['autodocs'],
	parameters: {
		docs: {
			description: {
				component:
					'Confirmation that the address on an account has changed. Sent by `SupervisorAddedEmailNotificationService.notifyEmailAddressChanged` today — an occasion the epic inventory missed, found while porting the senders.\n\nIt goes to the **new** address and states plainly what to do if it was not the account owner. Security class: no unsubscribe link, because an account change nobody can switch off is the point.'
			}
		}
	}
