እንኳዕ ናብ ${platformName!''} ብደሓን መጻእኩም
===================================

ን${tenantName!''} ናብ ምምሕዳር ${platformName!''} መእተዊ ተዳልዩ ኣሎ።

በቲ መላግቦ መሕለፊ ቃልኩም ተቐምጡ፣ ነቲ ምድላው ድማ ትውድእዎ።

ትካል: ${tenantName!''}
መላግቦ ዝሓልፈሉ: ${inviteExpiresAt!''}

መእተዊ ኣዳልዉ:
${inviteUrl!''}

ነዚ ዕድመ ዘይትጽበይዎ እንተነይርኩም፣ በጃኹም ነቲ ዝዓደመኩም ሰብ ርኸብዎ።

----------------------------------------------------------------
መሕለፊ ቃልኩም ብኢመይል ፈጺምና ኣይንሓትትን። ነዚ መላግቦ ንዝኾነ ሰብ ኣይትሃብዎ።

${orgName!''}
${orgAddress!''}
${contactLine!''}

${platformName!''} ብ${orgName!''} ዝቐርብ ኣገልግሎት እዩ።

ሓለዋ ዳታ: ${privacyUrl!''}
ሕጋዊ ሓበሬታ: ${imprintUrl!''}

እዛ ኢመይል ናይ ምእታው ኣካል ስለዝኾነት ክትስረዝ ኣይትኽእልን። በጃኹም ኣይትምለስዋ።
