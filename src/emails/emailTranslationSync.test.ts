import {
	bundledAppLocales,
	missingMailLocales
} from './scripts/appLocaleCoverage';
 ...
 *
 * The App catalogue defines the languages and the mail build must cover them. Adding the other
 ...
 *   4. a machine-translated language cannot be marked human-reviewed until the
...
describe('e-mail translations', () => {
	...
	describe('coverage', () => {
		it('covers every bundled App locale and rejects a new one without mail copy', () => {
			for (const locale of [
				...
				expect(bundledAppLocales()).toContain(locale);
			}
			expect(missingMailLocales(bundledAppLocales())).toEqual([]);
			expect(missingMailLocales([...bundledAppLocales(), 'uk'])).toEqual([
				'uk'
			]);
		});

		it.each([...EMAIL_LOCALES])(
			'%s has all catalogue occasions',
			(locale) => {
	...
	describe('human review status', () => {
		it('keeps untranslated review claims pending', () => {
