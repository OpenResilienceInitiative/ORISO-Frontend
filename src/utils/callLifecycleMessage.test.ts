...
describe('durable call lifecycle contract', () => {
	it('matches only the complete four-field call identity', () => {
		expect(hasSameCallLifecycleIdentity(call, { ...call })).toBe(true);
		for (const changed of [
			{ ...call, callId: 'call-2' },
			{ ...call, roomRef: '!other-source:example' },
			{ ...call, callRoomId: '!other-media:example' },
			{ ...call, callType: 'audio' as const }
		]) {
			expect(hasSameCallLifecycleIdentity(call, changed)).toBe(false);
		}
	});
