              ...
              <Box key={center.id}>
                ...
                >
                  <Radio
                    ...
                    sx={{ mt: -0.25, ml: -0.5, alignSelf: 'flex-start' }}
                  />
                  <ListItemText
                    primary={center.name}
                  ...
                  />
                  <IconButton
                    aria-label={t('reg.center.info')}
                    // Trailing affordance only — opening the detail panel is a
                    // separate concern, so the row click still drives selection.
                    onClick={(e) => {
                      e.stopPropagation();
                      /* no-op for now */
                    }}
                    sx={{ mt: -0.5, mr: -0.5, color: md3.onSurfaceVariant }}
                  >
                    <InfoOutlinedIcon />
                  </IconButton>
                </ListItemButton>
