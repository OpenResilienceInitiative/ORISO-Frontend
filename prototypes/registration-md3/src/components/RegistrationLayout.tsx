...
export default function RegistrationLayout({
  ...
  const nextButton = (
    <Button
      ...
      sx={{
        ...
        boxShadow: canContinue ? '0 6px 18px rgba(164,38,46,0.30)' : 'none',
  ...
  const selectionChip =
    selectedLabel != null ? (
      <Chip
        avatar={selectedIcon ? <Avatar src={selectedIcon} alt="" /> : undefined}
        label={selectedLabel}
  ...
  return (
    <Box sx={{ display: 'flex', height: '100dvh', overflow: 'hidden', bgcolor: '#fff' }}>
      ...
      >
        <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
          <Typography sx={{ fontSize: 42, fontWeight: 700, lineHeight: 1.1, mb: 1.5 }}>
            {t('app.brandTitle')}
          </Typography>
          <Typography sx={{ fontSize: 19, fontWeight: 400, opacity: 0.92 }}>
            {t('app.brandSubtitle')}
          </Typography>
        </Box>
        ...
        <Box sx={{ display: 'flex', gap: 1.5, mt: 3, fontSize: 12.5, opacity: 0.85, justifyContent: 'center' }}>
      ...
      >
        ...
        >
          <Typography sx={{ fontSize: 17, fontWeight: 700 }}>{t('app.brandTitle')}</Typography>
          <Typography sx={{ fontSize: 13, opacity: 0.85 }}>{t('app.brandSubtitle')}</Typography>
        </Box>
        ...
        <Stack
          ...
          spacing={1.5}
          sx={{ px: { xs: 2, md: 4 }, py: 1.5, flexShrink: 0, bgcolor: '#fff' }}
        >
          <LanguageSwitcher />
          <Button
            variant="outlined"
            startIcon={<LoginRoundedIcon />}
            sx={{ borderColor: md3.outlineVariant, color: md3.onSurface }}
          >
            {t('app.login')}
          </Button>
        </Stack>
        ...
        >
          <Box sx={{ maxWidth: CONTENT_MAX, mr: 'auto', pt: { xs: 2, md: 3 }, pb: 3 }}>
          ...
          <Box sx={{ maxWidth: CONTENT_MAX, mr: 'auto' }}>
            ...
            >
              ...
              >
                {selectionChip ? (
                  <>
                    <Typography sx={{ fontSize: 13, color: md3.onSurfaceVariant, flexShrink: 0 }}>
                      {t('reg.selectedLabel')}:
                    </Typography>
                    {selectionChip}
                  </>
                ) : (
                  <Typography sx={{ fontSize: 13, color: md3.outline }}>
                    {t('reg.noneSelected')}
                  </Typography>
                )}
              </Box>
