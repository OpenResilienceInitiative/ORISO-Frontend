...
describe('WP-06 list-item selection controller', () => {
	...
	describe('isListItemActive', () => {
		it('matches by sessionId, normalising number vs string', () => {
			...
			expect(isListItemActive(selection, { sessionId: 123 })).to.equal(
				true
			);
			expect(isListItemActive(selection, { sessionId: '123' })).to.equal(
				true
			);
			expect(isListItemActive(selection, { sessionId: 456 })).to.equal(
				false
			);
		});
		...
		it('matches by groupId or rid', () => {
			...
			expect(
				isListItemActive(selection, { groupId: 'AbCdGroup' })
			).to.equal(true);
			expect(isListItemActive(selection, { rid: 'AbCdGroup' })).to.equal(
				true
			);
			expect(
				isListItemActive(selection, { groupId: 'OtherGroup' })
			).to.equal(false);
		});
	...
	describe('exactly-one-active invariant', () => {
		...
		it('marks exactly one item active inside a conversation', () => {
			...
			expect(
				activeCount('/sessions/consultant/sessionView/g3/3')
			).to.equal(1);
		});
